...
const EditSermonPage = () => {
    ...
    const fetchSermon = async () => {
      try {
        ...
        setFormData({
          ...
          sermon_type: data.sermon_type as SermonType,
        ...
        if (data.youtube_video_id) {
          setYoutubePreview(getYouTubeThumbnailUrl(data.youtube_video_id, 'hq'))
        }
      } catch (error) {
  ...
  const handleSubmit = async (status: 'draft' | 'published') => {
    ...
    try {
      setSaving(true)
      await updateSermon(id as string, {
        ...formData,
        status
      })
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              <div className="flex items-center">
                ...
                <h1 className="text-xl font-bold text-gray-900 font-korean">설교 수정</h1>
              </div>
